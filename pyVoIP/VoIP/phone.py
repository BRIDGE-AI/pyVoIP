...
from pyVoIP.SIP.message.message import SIPMethod, SIPMessage, SIPRequest, SIPResponse
...
class VoIPPhone:
    ...
    def callback(
        ...
        if type(request) is SIPRequest:
            # debug(f"This is a message. request.method:{type(request.method)}, {request.method}")
